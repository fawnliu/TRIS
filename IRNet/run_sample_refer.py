...
if __name__ == '__main__':
    ...
    parser.add_argument("--voc12_root", default='./data/train2014', type=str,
    ...
    parser.add_argument("--cam_batch_size", default=26, type=int)
    ...
    if args.cam_to_ir_label_pass is True:
        import step_refer.cam_to_ir_label

        timer = pyutils.Timer('step.cam_to_ir_label:')
        step_refer.cam_to_ir_label.run(args)

    if args.train_irn_pass is True:
        import step_refer.train_irn

        timer = pyutils.Timer('step_refer.train_irn:')
        step_refer.train_irn.run(args)

    if args.make_sem_seg_pass is True:
        import step_refer.make_sem_seg_labels
        args.sem_seg_bg_thres = float(args.sem_seg_bg_thres)
        timer = pyutils.Timer('step_refer.make_sem_seg_labels:')
        step_refer.make_sem_seg_labels.run(args)
    ...
    if args.make_ins_seg_pass is True:
        import step_refer.make_ins_seg_labels

        timer = pyutils.Timer('step_refer.make_ins_seg_labels:')
        step_refer.make_ins_seg_labels.run(args)
