...
from misc.imutils import colorize_displacement
import cv2 
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            pack['size'] = [pack['size'][0][0].item(), pack['size'][1][0].item()]
            # print(pack['size'])
            ...
            # keys = cam_dict['keys']
            cams = torch.tensor(cam_dict).cuda() 
            # cams = cam_dict 
            keys = np.array([0])  
            ...
            cams = F.interpolate(cams.unsqueeze(0).unsqueeze(0), instance_map.shape[1:], align_corners=True, mode='bilinear').squeeze(0) ##!
...
def run(args):
    ...
    dataset = voc12.dataloader.ReferClassificationDatasetMSF(args.train_list,
