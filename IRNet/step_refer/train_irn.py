...
def run(args):
    ...
    train_dataset = voc12.dataloader.ReferAffinityDataset(img_name_list_path=args.train_list,
                                                          label_dir=args.ir_label_out_dir,
                                                          voc12_root=args.voc12_root,
                                                          indices_from=path_index.src_indices,
                                                          indices_to=path_index.dst_indices,
                                                          hor_flip=True,
                                                          crop_size=args.irn_crop_size, # 512
                                                          crop_method="random",
                                                          rescale=(0.5, 1.5))
    ...
    for ep in range(args.irn_num_epoches):
        ...
        for iter, pack in enumerate(train_data_loader):
            ...
            pos_aff_loss, neg_aff_loss, dp_fg_loss, dp_bg_loss = model(img, True) ####return_loss=True 
    ...
    infer_dataset = voc12.dataloader.ReferImageDataset(img_name_list_path=args.train_list, 
